...
var MONITOR; // holds list of child scriptName to PID and prevent more than one instance of script from running at  at time
var isMaster=false;
...
function executeChildThread(script,conf,cronDir,monitor){
  ...
  if(MONITOR[script]) {
    logger.alert("cron script "+script+" running with pid = "+MONITOR[script]+" has not terminated before its subsequent call.");
    return;
  }
  var child = cp.fork(__dirname+"/cron.js"); //child process of this same cron file
  var chPid = child.pid;
  PIDS[chPid]=script;
  MONITOR[script]=chPid;
  child.on("exit",function(code){
    var pid = chPid;
    delete MONITOR[PIDS[pid]];
    delete PIDS[pid];
  });
