  ...
  var halt=false;
  var cacheDuration=0;
  var cache = { //default empty cache definition to prevent null pointer errors
  ...
  exports.init = function(config){
    ...
    if(config.mode === "prod" && config.cache && config.cache.list){// cahe valid only for prod mode
      logger.trace("Memcache init startalled");
      var list = config.cache.list;
      cacheDuration=list.defaultCacheDuration;
      cache = new Memcached(list.servers,list.opts);  
      //TODO add code to verify if cache is up and running
      logger.trace("Memcache init end");
    }
  ...
  function waterfallProcess(cacheFlag,obj,debugFlag,response){
    try{
      ...
      validator.validate(obj,function(err,validReq){ // validate if corresponding json exist for this request
        if(err) response.end(build.error("validator",err.stack));
        else{
          ...
          if(debugFlag === "validate") { 
            ...
            cacheDuration = validReq.model.cacheDuration || cacheDuration; // cacheDuration can be customized for each js model
            query.build(validReq,function(err,queryReq){ // builds queries (main,count, joins)
              if(err){
                response.end(build.error("builder",err.stack));
                return; 
              }else{
                logger.trace(" query sucessfully built");
                if(debugFlag === "build"){
                ...
                query.execute(queryReq,function(err,resultObj){
                  if(err){
                    response.end(build.error("executor",err.stack));
                    return;
                  }else{
                    ...
                    logger.trace(" query sucessfully executed");
                    ...
                    formatter.format(formatObj,function(err,output){
                      if(err){
                        response.end(build.error("formatter",err.stack));
                        return;
                      }else{
                      logger.trace(" query sucessfully formatted");
                      ...
                      cache.set(cacheFlag,output,cacheDuration,function(err){
                        if(err) logger.error("Failed to set list cache",err.stack);
