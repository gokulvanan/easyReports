  ...
  exports.init = function(config){
    ...
    utils.fetchDaoPool =  function(db){
      return  query.getDao().getPool(db);
    }
        ...
        if(err) response.end(build.error("validator",err.stack));
          ...
          if(debugFlag === "validate") { 
            ...
            if(validReq.model.raw){ //added to provie interface to build raw query execution and formatting logic
              logger.debug("raw model case");
              try{
                validReq.model.raw(params,utils,rawUtils,function(err,output){
                  if(err){
                    response.end(build.error(err.msg,err.stack));
                  }else{
                ...
                response.end(build.err("raw model handler",err.stack));
              }
            }else{
              var cacheDuration = validReq.model.cacheDuration; // cacheDuration can be customized for each js model
