    ...
    var app = http.createServer(function(request,response){
      ...
      if(buff.length >= 2){
        ...
        for(var i=0; i<temp.length; i++){
                if (temp[i].indexOf("format") == 0 ){
                                var mimetemp = temp[i].split('=')[1];
                        }
        }
      build.setMime(mimetemp);
                ...
                if(err){
                  ...
                  query.execute(queryReq,function(err,resultObj){
                    if(err){
	                  ...
	                  logger.info("content type: "+params.format);
			
	                  if (params.format === "csv"){
	                        json2csv({ data: output.data, fields: Object.keys(queryReq.display) }, function(err, csv) {
	                          if (err) logger.error(err);
				  output.data=conf.filePath+params.csv_rand+".csv";
	                          fs.writeFile(conf.filePath+params.csv_rand+".csv", csv, function(err) {
	                            if (err) throw err;
	                          });
	                        });
				response.write(build.success(output));
