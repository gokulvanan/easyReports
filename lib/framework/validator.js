...
exports.validate  = function(req, cb){
  ...
  if(MODEL_MAP && MODEL_MAP[req.action]){ // prod mode has this cache map to improve performance
    ...
    validateParams(req,cb);
  }else{
    process.nextTick(function(){
      var model = MODEL_DIR + "/" + req.action + ".js";
      if (!fs.existsSync(model)){
        cb(new Error("Action path specified is invalid"));
      }else{
...
function validateParams(req,cb){
  ...
  logger.debug("initial params ="+JSON.stringify(params).substring(0,100));
  if(obj.filter){
    params = filterParams(obj.filter,req.search);
  }
  // apply defaults if needed
  logger.debug("filtered params ="+JSON.stringify(params).substring(0,100));
  ...
  logger.debug("final params ="+JSON.stringify(params));
