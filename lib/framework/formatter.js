...
exports.init = function(conf){
  ...
  var path = conf.app || "app";
  logic = require(process.cwd()+"/"+path);
  ...
  async.map(display, function(dp,callback){
    try{
      ...
      if(!dp.format){ // no format case
        ...
        // TODO support cache with join 
        var opts = dp.format.split("^");
        if(opts[0] === "cache"){ // memcache case
          var flag = opts[1];
          if(opts.length>2) {
              flag += common.getJoinFlag((j) ? j : row,opts.slice(2),"");
          }
          if(dp.key) flag += (j)? joinVal : row[dp.key];
          ...
          process.nextTick(function(){
            ...
            if(func){
              var val = (j) ? joinVal : (dp.key) ? row[dp.key] : null;
              returnObj.val= func(row,req.joins,val,opts.slice(1));
              callback(null,returnObj); 
            } 
