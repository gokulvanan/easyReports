    ...
    try{
      ...
      if(!dp.format){ // no format case
        ...
        if(opts[0] === "cache"){ // memcache case
          var flag = opts[1];
          if(opts.length>2) {
              flag += common.getJoinFlag((joinRow) ? joinRow : row,opts.slice(2),"");
          }
          if(dp.key) flag += (joinRow)? joinRow[dp.key] : row[dp.key];
          ...
          process.nextTick(function(){
            ...
            if(func){
              var val = (joinRow) ? joinRow[dp.key] : (dp.key) ? row[dp.key] : null;
              returnObj.val= func(row,req.results,val,opts.slice(1));
              callback(null,returnObj); 
            } 
