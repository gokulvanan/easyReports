...
function rowProcessorWrapper(display,req,row,cache,cb){
  var joins = req.joins;
  var joinRows={};
  var joinFlagMap={};
  for(var j in joins){
  ...
  req.display(req.params,row,joinRows,joinFlagMap,cache,cb); // display func needs row, joinRows and JoinFlagMap
}
...
function processDisplay(display,header,res,req,processRow,callback){
  async.map(res,function(row,cb){
    process.nextTick(function(){
      processRow(display,req,row,cache,cb);
    });
  },
  function (err,data){
    if(err) callback(err);
...
function update(display,req,row,cache,cb){
