...
(function(){
  ...
  program
    .version('0.0.1')
    .option('-s, --server', 'Http server only')
    .option('-c, --cron', 'Cron server only');
    ...
    if (!fs.existsSync(conf)){
      ...
      if(conf.logger){
        ...
        if(fs.existsSync(path)){
          common.removeDir(path, function(err){
            if(err){
              console.log("Unable to clean logs dir, Could not delete the old logs");
              process.exit(1);
            }else {
              console.log("deleted");
              common.createLogDir(path,function(err){
                console.log("here");
                if(err){
                  console.log("Unable to create new log directoies");
                  process.exit(1);
                }else{
                  console.log("All old logs have been cleaned");
                  process.exit(0);
                }
              });
            }
