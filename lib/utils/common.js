...
exports.createLogDir = function(path,cb){
	fs.mkdir(path,function(e){
    if(e)  cb(new Error("Error in creating "+path+". Check if folder already exist. Error msg: "+e));
    else {
      fs.mkdir(path+"/server",function(e){
        if(e) cb(new Error ("Error in creating"+path+"/server"));
        else{
          fs.mkdir(path+"/cron",function(e){
            if(e) cb(new Error ("Error in creating"+path+"/cron"));
            else  cb(null,"done");
          });
        }
      });
...
// remove director
exports.removeDir = function(path,cb){
