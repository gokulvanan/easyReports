...
function createLogDir(path){
    fs.mkdir(path,function(e){
      if(e){
        console.log("done creation 1");
        console.log("Error in creating "+path+". Check if folder already exist. Error msg: "+e);
        return;
      }else {
        fs.mkdir(path+"/server",function(e){
          if(e){
            console.log("Error in creating"+path+"/server");
            return;
          }else{
            fs.mkdir(path+"/cron",function(e){
              if(e){
                console.log("Error in creating"+path+"/cron");
              }else console.log("easy rep has cleaned the logs");
            });
          }
        });
      }
...
// remove director
exports.cleanLogs = function(path){
